...
void Arm::Periodic()
{
    frc::SmartDashboard::PutNumber("Position:Arm", m_fx.GetPosition().GetValueAsDouble());
}
...
bool Arm::IsMoveFinished(units::angle::turn_t desired) {
    return CONSTANTS::IN_THRESHOLD<units::turn_t>(m_fx.GetPosition().GetValue(), desired, 0.01_tr);
}

frc2::CommandPtr Arm::Move(units::angle::turn_t desired)
{
